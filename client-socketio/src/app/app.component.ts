...
export class AppComponent {
 ...
 sendMessage(formData: FormData) {
    ...
    switch (formData.messageType) {
      ...
      case "cesar":
        encoded.message = this.cesarService.encode(formData.message, Number(this.dfdf));
        break;
      //Se il tipo di messaggio è t-des allora cripto con cryptoService.encodeDes
      case "t-des":
        encoded.message = this.cryptoService.encodeDes(formData.message, this.dfdf);
        break;
    }
  ...
  decodeData = (messageData: string) => {
    ...
    switch (received.messageType) {
      case "cesar":
        received.message = this.cesarService.decode(received.message, Number(this.dfdf));
        break;

      case "t-des":
        received.message = this.cryptoService.decodeDes(received.message, this.dfdf);
        break;
    }
